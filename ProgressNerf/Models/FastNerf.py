...
class FastNerf(nn.Module):
    ...
    def forward_cache(self, pos, dir, uvws_cache:VoxelGrid, beta_cache:torch.tensor):
        ...
        idx_i = ((thetas.view(-1,1) + math.pi) * (l / (2*math.pi))).to(dtype=torch.long)
        idx_j = ((phis.view(-1,1) + math.pi) * (l / (2*math.pi))).to(dtype=torch.long)
            ...
            for x in tqdm(xs,leave=False):
                for y in tqdm(ys,leave=False):
                    for z in tqdm(zs,leave=False):
                        ...
                        uvws_result = self.forward(pos_encoder.encodeFeature(position), None, only_uvws=True)
                        if(uvws_result[...,-1:].item() > 0):
                            print("{0},{1},{2},{3}".format(x,y,z,uvws_result[...,-1:]))
                        uvws_cache.set_voxels_xyz(position, uvws_result)
