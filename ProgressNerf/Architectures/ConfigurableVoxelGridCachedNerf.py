...
import ProgressNerf.Dataloading.SemiSynDataloader
...
# of note, this architecture also allows for the use of configuration vectors in training/testing/evaluation
class ConfigurableVoxelGridCachedNerf(object):
    ...
    def parseConfig(self, config:Dict):
        ...
        conf_enc_dict = config['configEncoder']
        self.configuration_encoder = get_encoder(conf_enc_dict['encoder'])(conf_enc_dict[conf_enc_dict['encoder']])
        ...
        self.config_cache_bounds = torch.Tensor([float(val) for val in config['cache_config_bounds']]).view(-1,2).transpose(0,1).to(self.device)
    ...
    def render(self, ray_origins, ray_dirs, configurations, mark_visited_voxels = False, use_cache = False, config_cache_meta = {}):
        ...
        num_samples = ray_origins.shape[-2]
        configurations = configurations.unsqueeze(-2).repeat((1,num_samples,1))
        #print(sampled_locations)
        if use_cache:
            cache_nn = self.nn_fine if self.nn_fine is not None else self.nn
            configurations = configurations.unsqueeze(2).repeat(1,1,sampled_locations.shape[2],1)
            pos_confs = torch.cat((sampled_locations, configurations), dim=-1)
            cached_mlp_outputs = cache_nn.forward(pos_confs, ray_dirs, use_cache=True, uvws_cache = self.uvws_cache, beta_cache = self.beta_cache, config_cache_meta=config_cache_meta)
        ...
        encoded_configs = self.configuration_encoder.encodeFeature(configurations)
        encoded_configs = encoded_configs.unsqueeze(2).repeat(1,1,encoded_locs.shape[2],1)

        # compute deformations
        #deformations = self.nn.forward_conf_def(encoded_locs, encoded_configs)
        #sampled_locations += deformations
        #encoded_locs = self.pos_encoder.encodeFeature(sampled_locations)
        ...
        mlp_outputs = self.nn.forward(torch.cat([encoded_locs, encoded_configs],dim=-1), encoded_dirs_coarse) #(batch_size, num_rays, num_samples, 4)
    ...
    def doTrainRendering(self, sample_batched, mark_visited_voxels=True):
        ...
        train_configurations = sample_batched['configuration'].to(self.device) # (batch_size, n_config_dims)
        
        #cv.imshow("train img", train_imgs[0].cpu().numpy()[:,:,::-1])
        ...
        #cv.imshow("vox grid", voxel_grid_weights[0].cpu().numpy())
        #cv.waitKey(0)
        ...
        render_result = self.render(ray_origins, ray_dirs, train_configurations, mark_visited_voxels=mark_visited_voxels)
    ...
    def doTestRendering(self, sample_batched):
        ...
        cam_poses = torch.linalg.inv(sample_batched['{0}_pose'.format(self.tool)]).to(self.device) # (batch_size, 4, 4)
        configuration = sample_batched['configuration'].to(self.device) # (batch_size, n_conf_dims)
        full_rendering = self.doFullRender(cam_poses, configuration)
    ...
    # configurations should be a Tensor of size (N, n_dims) - comprised of N, n dimensional configuration vectors
    def doEvalRendering(self, camera_poses:torch.Tensor, configurations: torch.Tensor, use_cache=True):
        with torch.no_grad():
            # these are H,W ordere here, but get transposed in the render,etc. stages as necessary
            #
            rendering_output = self.doFullRender(camera_poses, configurations, use_cache = use_cache)
    ...
    # camera_poses: (batch_size, 4, 4)
    # configuration: (batch_size, n_conf_dims)
    def doFullRender(self, camera_poses:torch.Tensor, configuration:torch.Tensor, use_cache = False):
        ...
        full_rendering = {}
        #configuration = configuration.unsqueeze(-2).repeat((1, self.eval_subBatchSize, 1)) # (batch_size, eval_sumBatchSize, n_conf_dims)
        for subBatch_idx in range(num_subBatches):
            ...
            subBatch_rendering = self.render(subBatch_ray_origins, subBatch_ray_dirs,\
                configuration, use_cache=use_cache,\
                config_cache_meta={'num_config_dims': self.num_config_dims, 'config_cache_D': self.config_cache_D, 'config_bounds': self.config_cache_bounds})
    ...
    def train(self):
        ...
        if(self.doCacheBuild):
            ...
            self.nn.populate_grid(self.uvws_cache, self.beta_cache, self.pos_encoder, self.dir_encoder, self.configuration_encoder, self.config_cache_bounds, self.config_cache_D)
...
if __name__=="__main__":
    ...
    arch = ConfigurableVoxelGridCachedNerf(configFile)
    arch.train()